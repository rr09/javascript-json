//Data set included in a variable
var pokemonGo = {
  "pokemon": [{
    "id": 1,
    "num": "001",
...
var pokemon1 = pokemonGo.pokemon;          // accessing the pokemon object using . operator and storing it in a global variable

//Function to get the information of the Pokemon

var get_pokemonName = function(getPokemon) //declaration
{
  var isPokemonFound = false;  
  for (x in pokemon1)                     // looping through every object from the pokemon Array
  {   
    if (pokemon1[x].name==getPokemon)    // checking if the input pokemon name matches with that from the array
    {
      isPokemonFound = true;
      break;
    }
  ...
  if (isPokemonFound == true ) 
    {
      console.log(pokemon1[x]);         //displaying the information of the pokemon taken as the argument
    }
...
//Function to check for the next evolution field

var get_nextEvolution = function (getPokemon)
{ 
  var nextEvolutionFound = false;
  for (x in pokemon1)                       // looping through the pokemon array
  {  
    for (z in pokemon1[x].next_evolution)   // looping through the next evolution array
    { 
     if(pokemon1[x].next_evolution[z].name==getPokemon)  //Checking if the input name matches with that from the  next evolution array
      {
        nextEvolutionFound = true;
        console.log(pokemon1[x].name);;    //displaying the names of pokemon that have the inputted pokemon in their next evolution field
        
      }    
    }
  }  

  if (nextEvolutionFound == false ) 
    {
      alert ('No pokemon has ' +getPokemon+ '  in their “next_evolution” field.');
    }     
}

//Function to check for the weaknesses

var get_pokemonWeakness = function(getPokemon)
{
  var weaknessFound = false;
  for (x in pokemon1)
  { 
    for (y in pokemon1[x].weaknesses)      //looping through the weaknesses array
    {
      if (pokemon1[x].weaknesses[y]==getPokemon)  //checking if the input weakness matches with that from the weaknesses array
      {  
        weaknessFound = true;
        console.log(pokemon1[x].name);           //displaying the names of the pokemon that have the inputted weakness
      }       
    }       
  }    
  ...
  {
    alert ('Invalid Weakness');  
  } 
}
  
var name = prompt ('Enter a Pokemon name');     // Inputting pokemon name from the user
get_pokemonName(name);   // calling the get_pokemonName function
get_nextEvolution(name);  // calling the get_nextEvolution function 

var weakness = prompt ('Enter a Weakness');    //Inputting pokemon weakness from the user
get_pokemonWeakness(weakness);  // calling the get_pokemonWeakness function
